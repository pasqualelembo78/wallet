...
//usage: getApproximateBlockchainHeight("2026-05-14") or getApproximateBlockchainHeight("May 14 2026")
...
function getApproximateBlockchainHeight(_date, _nettype){
    // MevaCoin genesis: blocco 1 minato il 14 Maggio 2026 (timestamp verificato dalla chain)
    var mevacoinBirthTime = 1778788490;
    // MevaCoin usa un solo algoritmo, nessun fork V1->V2
    var secondsPerBlockV2 = 120;
    // time in UTC
    var requestedTime = Math.floor(new Date(_date) / 1000);
    var approxBlockchainHeight;
    var secondsPerBlock = secondsPerBlockV2;
    // prima della nascita di MevaCoin
    if (requestedTime < mevacoinBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < mevacoinBirthTime " );
        return 0;
    }
    // calcolo lineare dalla nascita della chain
    approxBlockchainHeight = Math.floor((requestedTime - mevacoinBirthTime) / secondsPerBlockV2);

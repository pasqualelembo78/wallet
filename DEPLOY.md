...
3. `git clone --recursive -b v0.X.Y.Z --depth 1 https://github.com/mevacoin-project/mevacoin-gui` 

4. Compile `mevacoin-wallet-gui.app`
...
5. Replace the `mevacoind` binary inside `mevacoin-wallet-gui.app/Contents/MacOS/` with one built using deterministic builds / gitian.
...
2. `codesign --deep --force --verify --verbose --options runtime --timestamp --entitlements entitlements.plist --sign 'XXXXXXXXXX' mevacoin-wallet-gui.app`

You can check if this step worked by using `codesign -dvvv mevacoin-wallet-gui.app`

3. `hdiutil create -fs HFS+ -srcfolder mevacoin-gui-v0.X.Y.Z -volname mevacoin-wallet-gui mevacoin-gui-mac-x64-v0.X.Y.Z.dmg`

4. `xcrun notarytool submit mevacoin-gui-mac-x64-v0.X.Y.Z.dmg --apple-id email@address.org --team-id XXXXXXXXXX`

5. `xcrun notarytool info aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeee --apple-id email@address.org --team-id XXXXXXXXXX`

6. `xcrun stapler staple -v mevacoin-gui-mac-x64-v0.X.Y.Z.dmg`
...
The `CMAKE_PREFIX_PATH` has to be set to `/path/to/qt-build-dir/` during mevacoin-gui compilation.

; MevaCoin Fluorine Fermi GUI Wallet Installer for Windows
; Copyright (c) 2017-2024, The MevaCoin Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\mevacoin-wallet-gui.exe")

[Setup]
AppName=MevaCoin GUI Wallet
...
DefaultDirName={commonpf}\MevaCoin GUI Wallet
DefaultGroupName=MevaCoin GUI Wallet
UninstallDisplayIcon={app}\mevacoin-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The MevaCoin Developer Community
AppPublisherURL=https://mevacoin.com
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the MevaCoin files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to MevaCoin.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; MevaCoin GUI wallet exe and guide
Source: "bin\mevacoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\mevacoin-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; MevaCoin CLI wallet
Source: "bin\extras\mevacoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; MevaCoin wallet RPC interface implementation
Source: "bin\extras\mevacoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; MevaCoin daemon
Source: "bin\mevacoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; MevaCoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "mevacoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; MevaCoin blockchain utilities
Source: "bin\extras\mevacoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mevacoin-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\mevacoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitmevacoin');
  s := 'The default folder to store the MevaCoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\mevacoin-project\mevacoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" MevaCoin default location
...
begin
  Result := BlockChainDir('') + '\bitmevacoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "mevacoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the MevaCoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\mevacoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\mevacoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "MevaCoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\mevacoin-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\mevacoin-gui-wallet-guide.pdf"; IconFilename: "{app}\mevacoin-wallet-gui.exe"
...
Name: "{group}\Utilities\MevaCoin Daemon"; Filename: "{app}\mevacoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\mevacoin-wallet-cli.exe"; WorkingDir: "{userdocs}\MevaCoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\MevaCoin\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\mevacoin-wallet-gui\mevacoin-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\mevacoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM mevacoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\mevacoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\mevacoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\mevacoin-project\mevacoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\mevacoin-project\mevacoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "mevacoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "mevacoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "mevacoin"; ValueType: "string"; ValueData: "URL:MevaCoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "mevacoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "mevacoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\mevacoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "mevacoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\mevacoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "mevacoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "mevacoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "mevacoinseed"; ValueType: "string"; ValueData: "URL:MevaCoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "mevacoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "mevacoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\mevacoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "mevacoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\mevacoin-wallet-gui.exe"" ""%1"""
